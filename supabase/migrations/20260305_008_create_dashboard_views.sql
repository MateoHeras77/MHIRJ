-- Phase 2 / Step 8: Aggregate views that power the 4 frontend pages.
-- All views are derived from v_flights_enriched so they stay in sync automatically.
...
create or replace view public.v_daily_volume as
select
  flight_date,
  count(*)::int as cnt
from public.v_flights_enriched
...
create or replace view public.v_airline_performance as
select
  airline_name,
...
create or replace view public.v_dow_volume as
select
  day_of_week,
  count(*)::int as cnt
from public.v_flights_enriched
...
create or replace view public.v_top_aircraft_models as
select
  aircraft_model,
  aircraft_category,
...
create or replace view public.v_airline_category_matrix as
select
  airline_name,
...
create or replace view public.v_opportunity_summary as
select
...
create or replace view public.v_opportunity_airlines as
select
  airline_name,
  count(*)::int                                                           as total_flights,
...
create or replace view public.v_opportunity_routes as
select
  route_code,
  airline_name,
  aircraft_model,
